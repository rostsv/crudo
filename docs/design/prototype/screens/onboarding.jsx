...
const TopBar = ({ step, total, onBack, showWordmark }) => (
  <div style={{
    flex: 'none', height: 76, padding: '32px 24px 0',
    display: 'flex', alignItems: 'center', justifyContent: showWordmark ? 'center' : 'space-between',
  }}>
    {showWordmark ? (
      <span style={{ fontWeight: 700, fontSize: 22, letterSpacing: '-0.8px', color: C.primary2 }}>Crudo</span>
    ) : (
      <>
        <button onClick={onBack} style={{
          width: 36, height: 36, border: 'none', background: 'transparent', cursor: 'pointer', padding: 0,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <svg width={9} height={14} viewBox="0 0 9 14" fill="none">
            <path d="M7 14L0 7L7 0L8.633 1.633L3.267 7L8.633 12.367L7 14Z" fill={C.primary2} />
          </svg>
        </button>
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          {Array.from({ length: total }).map((_, i) => (
            <div key={i} style={{
              ...
              width: i === step ? 18 : 6,
              background: i <= step ? C.primary : 'rgba(0,77,73,0.15)',
              transition: 'all .25s',
            }} />
          ))}
        </div>
...
const Body = ({ children, padding = '12px 24px 0' }) => (
...
const Sticky = ({ children }) => (
  <div style={{
    flex: 'none', padding: '16px 24px 32px',
    background: `linear-gradient(${C.bg} 30%, rgba(250,249,246,0.85) 100%)`,
...
const PrimaryBtn = ({ label, onClick, disabled, style = {} }) => (
  <button onClick={!disabled ? onClick : undefined} disabled={disabled} style={{
    width: '100%', height: 56, borderRadius: 40,
    background: `linear-gradient(${C.primary} 0%, ${C.primary2} 100%)`,
    boxShadow: '0 12px 32px rgba(0,77,73,0.18)',
    color: '#fff', fontFamily: 'Manrope', fontSize: 15, letterSpacing: '1px',
    fontWeight: 700, border: 'none', cursor: disabled ? 'default' : 'pointer',
...
const Headline = ({ kicker, title, sub }) => (
  <div style={{ marginBottom: 20, flex: 'none' }}>
    {kicker && <div style={{ fontSize: 11, letterSpacing: '1.2px', color: C.primary2, fontWeight: 700, marginBottom: 8 }}>{kicker}</div>}
    <div style={{ fontSize: 28, fontWeight: 800, lineHeight: '32px', letterSpacing: '-0.6px', color: C.text, textWrap: 'pretty' }}>{title}</div>
    {sub && <div style={{ fontSize: 14, color: C.text2, lineHeight: '20px', marginTop: 10, textWrap: 'pretty' }}>{sub}</div>}
  </div>
);
...
const AwarenessScreen = ({ onNext, onBack }) => (
  <Frame>
    <TopBar step={0} total={10} onBack={onBack} />
    <Body>
    ...
    <Sticky><PrimaryBtn label="CONTINUE" onClick={onNext} /></Sticky>
  </Frame>
);
...
const StructureScreen = ({ onNext, onBack }) => {
  ...
  return (
    <Frame>
      <TopBar step={1} total={10} onBack={onBack} />
      <Body>
...
const VideoDemoScreen = ({ onNext, onBack, onSkip }) => (
  <Frame>
    <TopBar step={2} total={10} onBack={onBack} />
    <Body>
...
const TransformationScreen = ({ onNext, onBack }) => (
  <Frame>
    <TopBar step={3} total={10} onBack={onBack} />
    <Body>
...
const HeardAboutScreen = ({ onNext, onBack, source, setSource }) => {
  ...
  return (
    <Frame>
      <TopBar step={4} total={10} onBack={onBack} />
      <Body>
      ...
      <Sticky><PrimaryBtn label="CONTINUE" onClick={onNext} disabled={!source} /></Sticky>
    </Frame>
  );
};
...
const TriedAppsScreen = ({ onNext, onBack, tried, setTried }) => {
  ...
  return (
    <Frame>
      <TopBar step={5} total={10} onBack={onBack} />
      <Body>
      ...
      <Sticky><PrimaryBtn label="CONTINUE" onClick={onNext} disabled={!tried} /></Sticky>
    </Frame>
  );
};
...
const BaselineScreen = ({ onNext, onBack, goal, setGoal }) => {
  ...
  return (
    <Frame>
      <TopBar step={6} total={10} onBack={onBack} />
      <Body>
...
const DailyStructureScreen = ({ onNext, onBack, mealCount, setMealCount }) => {
  ...
  return (
    <Frame>
      <TopBar step={7} total={10} onBack={onBack} />
      <Body>
...
const MealTimingScreen = ({ onNext, onBack, timing, setTiming }) => {
  ...
  return (
    <Frame>
      <TopBar step={8} total={10} onBack={onBack} />
      <Body>
...
const RemindersOBScreen = ({ onNext, onBack, reminders, setReminders }) => {
  ...
  return (
    <Frame>
      <TopBar step={9} total={10} onBack={onBack} />
      <Body>
